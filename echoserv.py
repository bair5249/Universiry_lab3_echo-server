...
def start_new_client(conn, addr):  # работа с отдельным клиентом
    while True:
        try:  # ВСЕ ЛОГИ ВЕДУ ЧЕРЕЗ TRY, ТАК КАК МОЖЕТ ВЫПОЛНЯТЬСЯ НЕСКОЛЬКО ПОПЫТОК ОТКРЫТИЯ ФАЙЛА,
            # ЛОГИ ПРЕКРАСНО ВЕДУТСЯ!
            ...
            if passwords[addr[0]] == data.decode():  # пароль совпал?
                ...
                while True:  # отныне этот клиент с нами в чате, пока он не напишет exit!
                    ...
                    for user in connected_users:
                        if user != conn:
                            user.send(
                                f'[{clients[addr[0]]}]:{msg.decode()}'.encode())  # отправляем всем, кроме отправителя
                    if msg.decode() == "exit":
                        for user in connected_users:
                            if user != conn:
...
while True:  # сервер, не спи, всегда внимательно слушай!
    ...
    try:
        log_file = open('server.log', 'a')  # пробуем открыть лог-файл на дописывание,
        # чтобы каждый новый клиент фиксировался
    except:
        pass
